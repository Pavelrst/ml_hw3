...
\includegraphics[width=.4\textwidth]{Winner_party_plots/MLP[50]_fig}
...
\includegraphics[width=.4\textwidth]{Division_prediction_plots/MLP[50]_fig}
\caption{Votes distribution}
\end{figure}

Looking at the plot we can clearly see that model performance corresponds to the chosen metrics, i.e. the euclidean norm. As stated in previous sections, the most similar histograms have the smallest euclidean distance. In conclusion, SVM classifier achieved best performance on validation set with final score of 69, which is the best score among other classifiers. 
...
\includegraphics[width=.4\linewidth]{Vote_prediction_plots/MLP[50]_fig}
...
In this section we will describe our implementation of bonus part of feature manipulation which will cause other party to win. To complete this task we've created a class $featureMAnipulator$ which automatically finds features which will cause other party to win.
