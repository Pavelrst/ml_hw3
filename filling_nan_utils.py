...
def fill_nans_by_lin_regress(train_set, val_set, test_set, verbose=True, graphic=False, all_history=False):
    ...
    numeric_features = train_set.select_dtypes(exclude=['object']).columns
    corr_matrix = train_set[numeric_features].corr()  # ignores string columns
    ...
    useful_features = set(numeric_features).difference(set(redundant_features))
